...
import {
  adjustData,
  labelFormatter,
  labelFormatterWithoutPercent,
} from "../../../../utils/pieChartHelpers";

const ChartsWrapper = ({ chartsData }: any) => {
  const healthStorage = chartsData?.healthStorage?.data?.data;
  const virtualMachines = chartsData?.virtualMachines?.data?.data?.sensors;
  const newVirtualMachines =
    chartsData?.newVirtualMachines?.data?.data?.sensors;
  const cpusStatus = chartsData?.chartNewCpu?.data?.data?.sensors;

  const user = useSelector((state: any) => state?.auth?.data?.user);

  console.log(chartsData?.virtualMachines?.data?.data?.sensors);
  ...
  const option = {
    ...
    legend: {
      ...
      left: "center",
      data:
        healthStorage?.length > 0
          ? healthStorage?.map((item: any) => item?.sensordata?.name)
          : [],
    },
    // tooltip: {
    //     trigger: 'item',
    ...
    series: [
      {
        ...
        data: adjustData(
          healthStorage?.length > 0
            ? healthStorage?.map((item: any) => ({
                value:
                  +item?.sensordata?.uptime?.split("%")[0] /
                  healthStorage?.length,
                name: item?.sensordata?.name,
                percentage: (+item?.sensordata?.uptime?.split("%")[0]).toFixed(
                  2
                ),
              }))
            : []
  ...
  console.log(virtualMachines?.map((item: any) => item?.sensor));
    ...
    legend: {
      ...
      data:
        virtualMachines?.length > 0
          ? virtualMachines?.map((item: any) => item?.sensor)
          : [],
    },
    series: [
      {
  ...
  const maxValue =
    newVirtualMachines?.length > 0
      ? Math.max(
          ...newVirtualMachines?.map((item: any) =>
            parseFloat(item?.lastvalue?.split(" %")[0])
          )
        )
      : 0;
  const barOption = {
    legend: {
      ...
      data:
        newVirtualMachines?.length > 0
          ? newVirtualMachines?.map((item: any) => item?.sensor)
          : [],
      // selected: {
      //     'Series 1': true,
      // },
