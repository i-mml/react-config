...
const DashboardConnectedDevices = ({ devices }: any) => {
    ...
    return (
        <div className={s.container}>
            ...
            {
                devices?.devices?.length > 0 && devices?.devices?.slice(0, 4)?.map((item: any, index: number) => <DeviceListItem key={item.objid} name={item.name} isLast={index === 3} status={item.fold} icon={item?.icon} />)
            }
