...
const LineChart = ({ data, netData }: any) => {
  ...
  const [rangeValue, setRangeValue] = useState<"UPLOAD" | "DOWNLOAD">(
    "DOWNLOAD"
  );
  ...
  const isDataEmpty =
    !Array.isArray(chartGraphData?.values) ||
    chartGraphData?.values?.length === 0;
  const option = {
    xAxis: {
      data: isDataEmpty
        ? []
        : chartGraphData?.values
            ?.slice(0, 50)
            ?.reverse()
            ?.map((item: any) =>
              moment(item?.datetime, "M/D/YYYY h:mm:ss").format("h:mm")
            ),
    ...
    center: ["60%", "40%"],
    legend: {
      data: ["ترافیک کل", "ترافیک ورودی", "ترافیک خروجی"],
      selectedMode: true,
      textStyle: {
        fontSize: 14,
        fontWeight: 500,
            ...
            data:
              ...
              chartGraphData?.values?.length > 0
                ? chartGraphData?.values
                    ...
                    ?.map(
                      (item: any) =>
                        Number(item?.["Traffic Out (volume)"]) / oneMilion
                    )
                : [],
            name: "ترافیک خروجی",
            type: "line",
            ...
            data:
              ...
              chartGraphData?.values?.length > 0
                ? chartGraphData?.values
                    ...
                    ?.map(
                      (item: any) =>
                        Number(item?.["Traffic In (volume)"]) / oneMilion
                    )
                : [],
            name: "ترافیک ورودی",
            type: "line",
            ...
            data:
              ...
              chartGraphData?.values?.length > 0
                ? chartGraphData?.values
                    ...
                    ?.reverse()
                    ?.map((item: any) =>
                      Number(item?.["Traffic Total (volume)"] / oneMilion)
                    )
                : [],
            name: "ترافیک کل",
            type: "line",
    ...
    series: [
      {
        ...
        detail: {
          ...
          formatter:
            `{value}` +
            `${
              data?.netSTatus?.data?.data?.channels
                ?.find((item: any) =>
                  rangeValue === "DOWNLOAD"
  ...
  useEffect(() => {
    setDownloadValue(
      +netData?.netSTatus?.data?.data?.channels
        ?.find((item: any) => item?.name === "Download Speed")
        ?.lastvalue.split(" ")[0]
        ?.split(",")
        ?.join("") || 0
    );
    setUploadValue(
      +netData?.netSTatus?.data?.data?.channels
        ?.find((item: any) => item?.name === "Upload Speed")
        ?.lastvalue.split(" ")[0]
        ?.split(",")
        ?.join("") || 0
    );
          ...
          <div className={s.speedTest}>
            ...
            <div className={s.speedBottom}>
              ...
              >
                ...
                >
                  <path
                    d="M62.4492 0.813152C53.7433 1.95925 50.9021 6.36578 42.0234 6.86332C33.8377 7.32204 29.7232 3.09327 21.5977 3.83824C11.744 4.74164 11.1275 12.5858 1.17187 12.9135"
                    stroke="#FE9B0E"
                    strokeLinecap="round"
              ...
              <div
                className={s.section}
                onClick={() => setRangeValue("DOWNLOAD")}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="60"
                ...
                >
                  <path
                    d="M59.2227 0.813152C50.9313 1.95925 48.2254 6.36578 39.7695 6.86332C31.9736 7.32204 28.055 3.09327 20.3164 3.83824C10.932 4.74164 10.3448 12.5858 0.863281 12.9135"
                    stroke="#007EFF"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
